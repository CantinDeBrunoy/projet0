<?php
  /********************************************************************************************************/
  /* SERVICE API QUI MODIFIE L'ETAT D'UN COMMENTAIRE EN 'signale' SELON L'ID DONNÉ (SQL -> etat='signale')*/
  /********************************************************************************************************/
  ...
  {
    global $conn;
    $query = "UPDATE commentaires SET etat='signale' WHERE idCom=$id";
    $result = mysqli_query($conn, $query);
    if($result) {
      $response=array(
        'status' => 1,
        'status_message' =>'Commentaire signalé avec succès.'
      );
    }
    encodeJSON($response);
  }
